...
extern void GC_Init(){
    ...
    malloc_verfy(GarbageCollector);
    malloc_verfy(GarbageCollector_Windows);
    malloc_verfy(GarbageCollector_Renderers);
    malloc_verfy(GC_ExtraCallable);
}

extern void GC_Append(void *item){
    GarbageCollector = realloc(GarbageCollector, GC_Count * sizeof(void*));
    error_identifier = ErrID_GC_Append; malloc_verfy(GarbageCollector);
...
extern void GC_AppendWindow(void *window){
    ...
    error_identifier = ErrID_GC_AppendW; malloc_verfy(GarbageCollector_Windows);
...
extern void GC_AppendRenderer(void *renderer){
    ...
    error_identifier = ErrID_GC_AppendR; malloc_verfy(GarbageCollector_Renderers);
...
extern void GC_AppendExtraCallable(void *callable){
    ...
    error_identifier = ErrID_GC_AppendC; malloc_verfy(GC_ExtraCallable);
